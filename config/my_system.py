...
def get_prob_info(
        control_model_file='', 
        plant_model_file='',
        perc=None, # [perc1, perc2, eps, seed]
        filter_val=None, 
        mpc_type='offsetfree',
    ):
    ...
    if perc is not None: 
        perc1, perc2, eps, seed = perc
        
        Ap_min = Ap - perc1 * abs(Ap)
        Ap_max = Ap + perc1 * abs(Ap) + eps
        Bp_min = Bp - perc2 * abs(Bp)
        Bp_max = Bp + perc2 * abs(Bp) + eps
        
        np.random.seed(seed = seed)
        
        Ap = Ap_min + np.random.rand(nx,nx)*(Ap_max - Ap_min)
        Bp = Bp_min +  np.random.rand(nx,nu)*(Bp_max - Bp_min)
    ...
    myref = lambda t: myRef(t, ts, ref=np.array([57.0,2980.0])) - xss[:nyc] # reference signal
    ...
    if perc is not None: 
        # Get the values of min, max values
        pert_dict = {}
        pert_dict['Ap_min'] = Ap_min
        pert_dict['Ap_max'] = Ap_max
        pert_dict['Bp_min'] = Bp_min
        pert_dict['Bp_max'] = Bp_max

        prob_info['pert_dict'] = pert_dict
